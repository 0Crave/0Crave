...
#<-----------------------------------------------------
    ...
    ckpt = os.path.join(model_dir, 'test_wrn101_9.pth')
    ckpt_weights = torch.load(ckpt)
    model = wide_resnet101_2(weights=None)
    model.fc = torch.nn.Linear(model.fc.in_features, 763)
...
# ------------------------------------------------------------------------>
    task_list = sorted(os.listdir(filepath), key=lambda x: int(x.replace('task', '')))  # 按数值大小排序
    def process_tasks(task_list):
        all_features = []
        all_labels = []
    
        for task_name in task_list:
            print(f"Processing task: {task_name}")  # Debug: 当前任务名称
            task_features = []
            task_labels = []
            task_path = os.path.join(filepath, task_name)  # 替换为实际路径
            
            # 处理 support 路径
            support_path = os.path.join(task_path, 'support')
            class_id = os.listdir(support_path)
            if os.path.exists(support_path) and os.path.isdir(support_path):
                # print(f"Found support path: {support_path}")  # Debug: 找到 support 路径
                for subfolder in os.listdir(support_path):
                    subfolder_path = os.path.join(support_path, subfolder)
                    # print(f"Processing support subfolder: {subfolder_path}")  # Debug: 当前子文件夹路径
                    if os.path.isdir(subfolder_path):
                        for filename in os.listdir(subfolder_path):
                            if filename.endswith(('.png', '.jpg', '.jpeg')):  # 根据需要调整支持的图像格式
                                image_path = os.path.join(subfolder_path, filename)
                                feature = extract_features(image_path)
                                if feature is not None:
                                    feature = feature.cpu()
                                    feature = feature.tolist()
                                    task_features.append(feature)  # 特征存储
                                    task_labels.append(int(subfolder))  # 标签存储
                                    # print(f"Extracted feature from: {image_path}")  # Debug: 提取特征的文件
    ...
    ndatas,labels = process_tasks(task_list)
    labels = torch.tensor(labels)
    ndatas = torch.tensor(ndatas)
    print(labels)
    # <------------------------------------------------------------------->
    # Power transform
    beta = 0.5
    ...
    n_runs = count_subdirectories(filepath)
    print("n_runs",n_runs)
    ...
    acc_test,labels = optim.loop(model, ndatas, n_runs, n_ways, n_usamples, n_lsamples, n_epochs=50)
    print(labels)
    # print(new_labels)
    result = labels[:, n_lsamples:]
#<-------------------------------------------------------------------------------
    res = ['img_name,label']  # 初始化结果文件，定义表头
    # ------------------------------------------------------------------------>
    ...
    for task_idx, task_name in enumerate(task_list):
        ...
        for path_idx, pathi in enumerate(test_img_lst):
            ...
            label = result[task_idx, path_idx].item()  # 获取对应的标签
...
n_runs = count_subdirectories('testA')
